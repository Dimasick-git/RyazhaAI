...
public:
    ...
    virtual tsl::elm::Element* createUI() override {
        ...
        auto* Status = new tsl::elm::CustomDrawer([this](tsl::gfx::Renderer *renderer, u16 x, u16 y, u16 w, u16 h) {
            ...
            const char* fpsPart = (FPSavg != 254.0) ? FPSavg_c : "--";
            const char* hzPart = RefreshRate_c;
            static constexpr const char* kFpsPrefix = "FPS:";
            static constexpr const char* kSep = " | ";

            const auto [fpsPrefixW, textHeight] = renderer->getTextDimensions(kFpsPrefix, false, fontsize);
            const auto [fpsW, fpsH] = renderer->getTextDimensions(fpsPart, false, fontsize);
            const auto [sepW, sepH] = renderer->getTextDimensions(kSep, false, fontsize);
            const auto [hzW, hzH] = renderer->getTextDimensions(hzPart, false, fontsize);

            const size_t margin = (fontsize / 8);
            
            // Inner rectangle dimensions (content area)
            const size_t innerWidth = fpsPrefixW + fpsW + sepW + hzW + margin + (margin / 2);
            ...
            actualTextWidth = fpsPrefixW + fpsW + sepW + hzW;
            ...
            int curX = textX;
            renderer->drawString(kFpsPrefix, false, curX, textY, fontsize, settings.textColor);
            curX += fpsPrefixW;
            renderer->drawString(fpsPart, false, curX, textY, fontsize, fpsColor);
            curX += fpsW;
            renderer->drawString(kSep, false, curX, textY, fontsize, settings.textColor);
            curX += sepW;
            renderer->drawString(hzPart, false, curX, textY, fontsize, hzColor);
        });
    ...
    virtual void update() override {
        ...
        // Update Hz from SaltySD periodically so it changes in real time.
        {
            const u64 nowNs = armTicksToNs(armGetSystemTick());
            static constexpr u64 kHzUpdatePeriodNs = 1'000'000'000ULL;

            if (lastHzUpdateNs == 0 || (nowNs - lastHzUpdateNs) >= kHzUpdatePeriodNs) {
                lastHzUpdateNs = nowNs;
                if (!SaltySD_Connect()) {
    ...
    virtual bool handleInput(u64 keysDown, u64 keysHeld, const HidTouchState &touchPos, HidAnalogStickState joyStickPosLeft, HidAnalogStickState joyStickPosRight) override {
        ...
        if (!isDragging) {
            if (isKeyComboPressed(keysHeld, keysDown)) {
                ...
                skipOnce = true;
                TeslaFPS = 60;
                lastSelectedItem = "Счетчик ФПС|Гц";
                lastMode = "";
                if (skipMain) {
                    lastMode = "return";
                    tsl::goBack();
                    ...
                    tsl::setNextOverlay(filepath.c_str(), "--lastSelectedItem 'Счетчик ФПС|Гц'");
