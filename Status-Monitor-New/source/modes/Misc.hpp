        ...
        auto* Status = new tsl::elm::CustomDrawer([this](tsl::gfx::Renderer *renderer, u16 x, u16 y, u16 w, u16 h) {
            ...
            if (R_SUCCEEDED(nvdecCheck | nvencCheck | nvjpgCheck)) {
                ...
                if (R_SUCCEEDED(nvdecCheck)) {
                    renderer->drawString("ЦПУ", false, X_OFFSET+15, currentY, 15, (settings.catColor2));
                    renderer->drawString(NVDEC_value_c, false, NV_VALUE_X, currentY, 15, (settings.textColor));
                    currentY += 15;
                }
                if (R_SUCCEEDED(nvencCheck)) {
                    renderer->drawString("Видеокод", false, X_OFFSET+15, currentY, 15, (settings.catColor2));
                    renderer->drawString(NVENC_value_c, false, NV_VALUE_X, currentY, 15, (settings.textColor));
                    currentY += 15;
                }
                if (R_SUCCEEDED(nvjpgCheck)) {
                    renderer->drawString("Видеодек", false, X_OFFSET+15, currentY, 15, (settings.catColor2));
            ...
            if (R_SUCCEEDED(nifmCheck)) {
                ...
                if (!Nifm_internet_rc) {
                    if (NifmConnectionType == NifmInternetConnectionType_WiFi) {
                        ...
                        if (!Nifm_profile_rc) {
                            if (Nifm_showpass)
                                ...
                                renderer->drawString("Нажми Y чтобы показать пароль", false, X_OFFSET, 305, 15, (settings.textColor));
                        }
                    }
                    ...
                    renderer->drawString("Тип: Нет сети", false, X_OFFSET, 280, 18, (settings.textColor));
            }
