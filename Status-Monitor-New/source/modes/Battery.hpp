...
public:
    ...
    virtual tsl::elm::Element* createUI() override {
        ...
        auto* Status = new tsl::elm::CustomDrawer([this](tsl::gfx::Renderer *renderer, u16 x, u16 y, u16 w, u16 h) {
            ...
            renderer->drawString("Статистика батареи", false, LABEL_X, 120 + Y_OFFSET, 20, LABEL_COLOR_1);
            
            u16 currentY = START_Y;
            
            // Actual Capacity
            renderer->drawString("Факт. ёмкость", false, LABEL_X, currentY, FONT_SIZE, LABEL_COLOR_2);
            renderer->drawString(actualCapacity_c, false, VALUE_X, currentY, FONT_SIZE, VALUE_COLOR);
            currentY += LINE_HEIGHT;
            
            // Designed Capacity
            renderer->drawString("Номин. ёмкость", false, LABEL_X, currentY, FONT_SIZE, LABEL_COLOR_2);
            renderer->drawString(designedCapacity_c, false, VALUE_X, currentY, FONT_SIZE, VALUE_COLOR);
            currentY += LINE_HEIGHT;
            
            // Temperature
            renderer->drawString("Температура", false, LABEL_X, currentY, FONT_SIZE, LABEL_COLOR_2);
            renderer->drawString(batteryTemp_c, false, VALUE_X, currentY, FONT_SIZE, VALUE_COLOR);
            currentY += LINE_HEIGHT;
            
            // Raw Charge
            renderer->drawString("Сырой заряд", false, LABEL_X, currentY, FONT_SIZE, LABEL_COLOR_2);
            renderer->drawString(rawCharge_c, false, VALUE_X, currentY, FONT_SIZE, VALUE_COLOR);
            currentY += LINE_HEIGHT;
            
            // Age
            renderer->drawString("Износ", false, LABEL_X, currentY, FONT_SIZE, LABEL_COLOR_2);
            renderer->drawString(batteryAge_c, false, VALUE_X, currentY, FONT_SIZE, VALUE_COLOR);
            currentY += LINE_HEIGHT;
            
            // Voltage
            renderer->drawString("Напряжение", false, LABEL_X, currentY, FONT_SIZE, LABEL_COLOR_2);
            renderer->drawString(voltageAvg_c, false, VALUE_X, currentY, FONT_SIZE, VALUE_COLOR);
            currentY += LINE_HEIGHT;
            
            // Current Flow
            renderer->drawString("Ток", false, LABEL_X, currentY, FONT_SIZE, LABEL_COLOR_2);
            renderer->drawString(currentFlow_c, false, VALUE_X, currentY, FONT_SIZE, VALUE_COLOR);
            currentY += LINE_HEIGHT;
            
            // Power Flow
            renderer->drawString("Мощность", false, LABEL_X, currentY, FONT_SIZE, LABEL_COLOR_2);
            renderer->drawString(powerFlow_c, false, VALUE_X, currentY, FONT_SIZE, VALUE_COLOR);
            currentY += LINE_HEIGHT;
            
            // Remaining Time
            renderer->drawString("Осталось", false, LABEL_X, currentY, FONT_SIZE, LABEL_COLOR_2);
            ...
            if (isChargerConnected) {
                ...
                renderer->drawString("Зарядка", false, LABEL_X, currentY, 20, LABEL_COLOR_1);
                currentY += 2*LINE_HEIGHT;
                // Input Current Limit
                renderer->drawString("Лимит тока (вх.)", false, LABEL_X, currentY, FONT_SIZE, LABEL_COLOR_2);
                renderer->drawString(inputCurrentLimit_c, false, VALUE_X, currentY, FONT_SIZE, VALUE_COLOR);
                currentY += LINE_HEIGHT;
                
                // VBUS Current Limit
                renderer->drawString("Лимит VBUS", false, LABEL_X, currentY, FONT_SIZE, LABEL_COLOR_2);
                renderer->drawString(vbusCurrentLimit_c, false, VALUE_X, currentY, FONT_SIZE, VALUE_COLOR);
                currentY += LINE_HEIGHT;
                
                // Charge Voltage Limit
                renderer->drawString("Лимит напр.", false, LABEL_X, currentY, FONT_SIZE, LABEL_COLOR_2);
                renderer->drawString(chargeVoltageLimit_c, false, VALUE_X, currentY, FONT_SIZE, VALUE_COLOR);
                currentY += LINE_HEIGHT;
                
                // Charge Current Limit
                renderer->drawString("Лимит тока", false, LABEL_X, currentY, FONT_SIZE, LABEL_COLOR_2);
                renderer->drawString(chargeCurrentLimit_c, false, VALUE_X, currentY, FONT_SIZE, VALUE_COLOR);
                currentY += LINE_HEIGHT;
                
                // Charger Type
                renderer->drawString("Тип", false, LABEL_X, currentY, FONT_SIZE, LABEL_COLOR_2);
                renderer->drawString(chargerType_c, false, VALUE_X, currentY, FONT_SIZE, VALUE_COLOR);
                currentY += LINE_HEIGHT;
                
                // Charger Max Voltage
                renderer->drawString("Макс. напр.", false, LABEL_X, currentY, FONT_SIZE, LABEL_COLOR_2);
                renderer->drawString(chargerMaxVoltage_c, false, VALUE_X, currentY, FONT_SIZE, VALUE_COLOR);
                currentY += LINE_HEIGHT;
                
                // Charger Max Current
                renderer->drawString("Макс. ток", false, LABEL_X, currentY, FONT_SIZE, LABEL_COLOR_2);
                renderer->drawString(chargerMaxCurrent_c, false, VALUE_X, currentY, FONT_SIZE, VALUE_COLOR);
            }
        });

        //tsl::elm::g_disableMenuCacheOnReturn.store(true, std::memory_order_release);
        tsl::elm::HeaderOverlayFrame* rootFrame = new tsl::elm::HeaderOverlayFrame("Ryazha-Status-Monitor", APP_VERSION, true);
    ...
    virtual bool handleInput(uint64_t keysDown, uint64_t keysHeld, touchPosition touchInput, JoystickPosition leftJoyStick, JoystickPosition rightJoyStick) override {
        if (keysDown & KEY_B) {
            ...
            skipOnce = true;
            runOnce = true;
            lastSelectedItem = "Батарея/Зарядка";
            lastMode = "";
