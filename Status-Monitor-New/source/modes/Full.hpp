    ...
    virtual tsl::elm::Element* createUI() override {
        ...
        auto Status = new tsl::elm::CustomDrawer([this](tsl::gfx::Renderer *renderer, u16 x, u16 y, u16 w, u16 h) {
            ...
            if (R_SUCCEEDED(clkrstCheck) || R_SUCCEEDED(pcvCheck)) {
                ...
                renderer->drawString("Загрузка CPU", false, COMMON_MARGIN, 120, 20, (settings.catColor1));
                if (settings.showTargetFreqs) {
                    //static auto targetFreqWidth = renderer->getTextDimensions("Target Frequency: ", false, 15).first;
                    renderer->drawString("Целевая частота", false, COMMON_MARGIN, height_offset, 15, (settings.catColor2));
                    renderer->drawString(CPU_Hz_c, false, COMMON_MARGIN + valueOffset, height_offset, 15, (settings.textColor));
                }
                if (realCPU_Hz && settings.showRealFreqs) {
                    //static auto realFreqWidth = renderer->getTextDimensions("Real Frequency: ", false, 15).first;
                    renderer->drawString("Реальная частота", false, COMMON_MARGIN, height_offset - 15, 15, (settings.catColor2));
                ...
                static auto core0Width = renderer->getTextDimensions("Ядро 0  ", false, 15).first;
                static auto core1Width = renderer->getTextDimensions("Ядро 1  ", false, 15).first;
                static auto core2Width = renderer->getTextDimensions("Ядро 2  ", false, 15).first;
                static auto core3Width = renderer->getTextDimensions("Ядро 3  ", false, 15).first;
                
                const uint32_t core_height = height_offset + 30;
                renderer->drawString("Ядро 0  ", false, COMMON_MARGIN, core_height, 15, (settings.catColor2));
                renderer->drawString(CPU_Core0_c, false, COMMON_MARGIN + core0Width, core_height, 15, (settings.textColor));
                
                renderer->drawString("Ядро 1  ", false, COMMON_MARGIN, core_height + 15, 15, (settings.catColor2));
                renderer->drawString(CPU_Core1_c, false, COMMON_MARGIN + core1Width, core_height + 15, 15, (settings.textColor));
                
                renderer->drawString("Ядро 2  ", false, COMMON_MARGIN, core_height + 30, 15, (settings.catColor2));
                renderer->drawString(CPU_Core2_c, false, COMMON_MARGIN + core2Width, core_height + 30, 15, (settings.textColor));
                
                renderer->drawString("Ядро 3  ", false, COMMON_MARGIN, core_height + 45, 15, (settings.catColor2));
            ...
            if (R_SUCCEEDED(clkrstCheck) || R_SUCCEEDED(pcvCheck) || R_SUCCEEDED(nvCheck)) {
                ...
                renderer->drawString("Загрузка GPU", false, COMMON_MARGIN, 285-8, 20, (settings.catColor1));
                if (R_SUCCEEDED(clkrstCheck) || R_SUCCEEDED(pcvCheck)) {
                    if (settings.showTargetFreqs) { 
                        //static auto targetFreqWidth = renderer->getTextDimensions("Target Frequency: ", false, 15).first;
                        renderer->drawString("Целевая частота", false, COMMON_MARGIN, height_offset, 15, (settings.catColor2));
                ...
                if (R_SUCCEEDED(nvCheck)) {
                    ...
                    renderer->drawString("Нагрузка", false, COMMON_MARGIN, height_offset + 15, 15, (settings.catColor2));
            ...
            if (R_SUCCEEDED(clkrstCheck) || R_SUCCEEDED(pcvCheck) || R_SUCCEEDED(Hinted)) {
                ...
                renderer->drawString("Загрузка RAM", false, COMMON_MARGIN, 375, 20, (settings.catColor1));
                if (R_SUCCEEDED(clkrstCheck) || R_SUCCEEDED(pcvCheck)) {
                    if (settings.showTargetFreqs) {
                        //static auto targetFreqWidth = renderer->getTextDimensions("Target Frequency: ", false, 15).first;
                        renderer->drawString("Целевая частота", false, COMMON_MARGIN, height_offset, 15, (settings.catColor2));
                ...
                if (R_SUCCEEDED(Hinted)) {
                    ...
                    renderer->drawString("Всего\nПриложение\nАпплет\nСистема\nСист. (небез.)", false, COMMON_MARGIN, height_offset + 40, 15, (settings.catColor2));
            ...
            if (R_SUCCEEDED(i2cCheck) || R_SUCCEEDED(tcCheck) || R_SUCCEEDED(pwmCheck)) {
                renderer->drawString("Плата", false, 20, 550+2, 20, (settings.catColor1));
                if (R_SUCCEEDED(i2cCheck)) {
                    renderer->drawString("Мощность бат.", false, COMMON_MARGIN, 575+2, 15, (settings.catColor2));
                ...
                if (R_SUCCEEDED(pwmCheck)) {
                    renderer->drawString("Обороты вентиля", false, COMMON_MARGIN, 590+2, 15, (settings.catColor2));
                ...
                if (R_SUCCEEDED(i2cCheck) || R_SUCCEEDED(tcCheck)) {
                    static auto socLabelWidth = renderer->getTextDimensions("СИС  ", false, 15).first;
                    static auto pcbLabelWidth = renderer->getTextDimensions("ПЛАТА  ", false, 15).first;
                    static auto maxLabelWidth = std::max(socLabelWidth, pcbLabelWidth);
                    static auto skinLabelWidth = renderer->getTextDimensions("Корпус  ", false, 15).first;
                    ...
                    renderer->drawString("Температуры", false, COMMON_MARGIN, 605+2, 15, (settings.catColor2));
                    
                    // SOC - starts at valueOffset next to "Temperatures"
                    uint32_t current_x = COMMON_MARGIN + valueOffset;
                    renderer->drawString("СИС  ", false, current_x, 605+2, 15, (settings.catColor2));
                    ...
                    renderer->drawString("Корпус  ", false, current_x, 605+2, 15, (settings.catColor2));
                    ...
                    renderer->drawString("ПЛАТА  ", false, current_x, 620+2, 15, (settings.catColor2));
                    current_x += maxLabelWidth;
            ...
            if (GameRunning) {
                ...
                if (settings.showFPS || settings.showRES || settings.showRDSD) {
                    renderer->drawString("Игра", false, COMMON_MARGIN + width_offset, 185+12, 20, (settings.catColor1));
                }
                uint32_t height = 210+12;
                if (settings.showFPS == true) {
                    static auto pfpsWidth = renderer->getTextDimensions("ПЧаст  ", false, 15).first;
                    static auto fpsWidth = renderer->getTextDimensions("Част  ", false, 15).first;
                    
                    renderer->drawString("ПЧаст  ", false, COMMON_MARGIN + width_offset, height, 15, (settings.catColor2));
                    const tsl::Color pfpsColor = getFpsColor((float)FPS);
                    renderer->drawString(PFPS_value_c, false, COMMON_MARGIN + width_offset + pfpsWidth, height, 15, pfpsColor);
                    ...
                    renderer->drawString("Част  ", false, fps_x_offset, height, 15, (settings.catColor2));
                    const tsl::Color fpsColor = getFpsColor(useOldFPSavg ? FPSavg_old : FPSavg);
                    renderer->drawString(FPS_value_c, false, fps_x_offset + fpsWidth, height, 15, fpsColor);
                    
                    height += 15;
                }
                if ((settings.showRES == true) && NxFps && SharedMemoryUsed && (NxFps -> API >= 1)) {
                    static auto resLabelWidth = renderer->getTextDimensions("Разрешение  ", false, 15).first;
                    renderer->drawString("Разрешение  ", false, COMMON_MARGIN + width_offset, height, 15, (settings.catColor2));
                ...
                if (settings.showRDSD == true) {
                    static auto readLabelWidth = renderer->getTextDimensions("Скор. чтения  ", false, 15).first;
                    renderer->drawString("Скор. чтения  ", false, COMMON_MARGIN + width_offset, height, 15, (settings.catColor2));
            ...
            static const auto pressWidth = renderer->getTextDimensions("Нажми ", false, 23).first;
            ...
            // Draw "Press "
            renderer->drawString("Нажми ", false, baseX, baseY, fontSize, (tsl::bottomTextColor));
            ...
            // Draw " to Exit"
            renderer->drawString(" для выхода", false, baseX + pressWidth + keyComboWidth, baseY, fontSize, (tsl::bottomTextColor));
        });
        
        auto rootFrame = new tsl::elm::HeaderOverlayFrame("Ryazha-Status-Monitor", APP_VERSION);
