...
ALWAYS_INLINE tsl::Color clampColor4444(int r, int g, int b, int a = 0xF) {
    r = std::clamp(r, 0, 15);
    g = std::clamp(g, 0, 15);
    b = std::clamp(b, 0, 15);
    a = std::clamp(a, 0, 15);
    return tsl::Color((u8)r, (u8)g, (u8)b, (u8)a);
}

ALWAYS_INLINE tsl::Color gradientRedToGreen(float value, float minValue, float maxValue) {
    if (!(maxValue > minValue)) {
        return clampColor4444(15, 15, 15);
    }
    const float t = std::clamp((value - minValue) / (maxValue - minValue), 0.0f, 1.0f);
    const int r = (int)lroundf(15.0f * (1.0f - t));
    const int g = (int)lroundf(15.0f * t);
    return clampColor4444(r, g, 0);
}

ALWAYS_INLINE tsl::Color gradientGreenToRed(float value, float minValue, float maxValue) {
    if (!(maxValue > minValue)) {
        return clampColor4444(15, 15, 15);
    }
    const float t = std::clamp((value - minValue) / (maxValue - minValue), 0.0f, 1.0f);
    const int r = (int)lroundf(15.0f * t);
    const int g = (int)lroundf(15.0f * (1.0f - t));
    return clampColor4444(r, g, 0);
}

ALWAYS_INLINE tsl::Color getFpsColor(float fps) {
    if (!(fps > 0.0f) || fps >= 254.0f) {
        return clampColor4444(15, 15, 15);
    }
    return gradientRedToGreen(fps, 0.0f, 60.0f);
}

ALWAYS_INLINE tsl::Color getHzColor(float hz) {
    if (!(hz > 0.0f)) {
        return clampColor4444(15, 15, 15);
    }
    if (hz <= 60.0f) {
        return gradientRedToGreen(hz, 0.0f, 60.0f);
    }
    return gradientGreenToRed(hz, 60.0f, 120.0f);
}
